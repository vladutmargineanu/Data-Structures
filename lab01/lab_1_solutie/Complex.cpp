...
int main() {
	...
	Complex sum2 = c2.add(c2);  //adunearea c2 cu c2(cu el insusi)

	out << sum1.getRe() << " " << sum1.getIm() << std::endl;
	out << sum2.getRe() << " " << sum2.getIm() << std::endl;

	Complex prod1 = c1.multiply(c2);  //inmultirea c1 cu c2
	Complex prod2 = c2.multiply(prod1); //inmultirea c2 cu prod1
