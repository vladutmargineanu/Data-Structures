...
public:
    ...
    static std::vector<int> BFS(ListGraph listGraph) {
        ...
        // initializam vecorul visited cu false (peste tot)
        std::vector<bool> visited = std::vector<bool>(size, false);
        // vectorul bfs retine arborele
        std::vector<int> bfs = std::vector<int>();
        // in coada retinem nodurile
        std::queue<int> q;
        q.push(0);
        visited[0] = true;
    ...
    static std::vector<std::vector<int>> Floyd_Warshall(MatrixGraph matrixGraph) {
        ...
        // initializam toate distantele cu 0
        dist.resize(size, std::vector<int>(size, 0));

        for (int i = 0; i < size; i++) {
            for (int j = 0; j < size; j++) {
                // Initializam matricea costurlor (distantele)
                dist[i][j] = matrixGraph.edgeCost(i, j);
            }
        }
        // pentru fiecare nod k (orice nod al grafului)
        for (int k = 0; k < size; k++) {
            // pentru orice pereche de noduri din graf i si j
            for (int i = 0; i < size; i++) {
                for (int j = 0; j < size; j++) {
